...
| Remote history loading | Missing | macOS serves opt-in recent-message pages, revision-based conditional reads, stable older-history cursors, and explicit full-message details. AgentGateway and Mac/browser Remote use bounded previews, idle polling backoff, cached history, and independent tab-list/conversation error state; old full-snapshot clients remain compatible. |
